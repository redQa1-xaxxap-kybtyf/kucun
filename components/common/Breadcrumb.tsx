...
export function Breadcrumb({
  ...
  showHome = true,
  className,
  separator = <ChevronRight className="text-slate-300 h-3.5 w-3.5" />,
}: BreadcrumbProps) {
    ...
    segments.forEach((segment, index) => {
      ...
      if (/^[0-9a-f-]{36}$|^\d+$/.test(segment)) {
        if (isLast && dynamicTitle) {
          ...
          const mappedDetailTitle = parentTitle ? DETAIL_TITLE_MAP[parentTitle] : undefined;
      ...
      if (segment === 'edit' && index >= 2) {
        ...
        const mappedEditTitle = grandParentTitle ? EDIT_TITLE_MAP[grandParentTitle] : undefined;
  ...
  }, [pathname, items, showHome, dynamicTitle]);
      ...
      <ol className="flex items-center gap-1.5">
        {autoGeneratedItems.map((item, index) => (
          <li key={index} className="flex items-center gap-1.5">
            {index > 0 && (
              <span className="flex items-center cursor-default" aria-hidden="true">
                {separator}
              </span>
            )}
            ...
            {item.isCurrent ? (
              <span 
                className="text-slate-900 text-xs font-black tracking-tight px-2 py-0.5 rounded-md bg-slate-100/50" 
                aria-current="page"
              >
                {item.title}
              </span>
              ...
              <Link
                href={item.href}
                className="group flex items-center gap-1 text-slate-400 hover:text-slate-900 hover:bg-slate-100/30 px-2 py-0.5 rounded-md transition-all active:scale-95"
              >
                {index === 0 && showHome && <Home className="h-3.5 w-3.5 opacity-60 transition-all group-hover:opacity-100 group-hover:scale-110" />}
                <span className="text-xs uppercase font-black tracking-wider">{item.title}</span>
              </Link>
            ) : (
              <span className="text-slate-400 font-bold px-2 py-0.5">{item.title}</span>
            )}
          </li>
        ))}
      </ol>
