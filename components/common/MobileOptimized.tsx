...
export function MobileList<T>({
  ...
  const isMobile = useMediaQuery('(max-width: 768px)')
  const [swipeStates, setSwipeStates] = React.useState<Record<number, {
    startX: number
    currentX: number
    isSwiping: boolean
  }>>({})
  ...
  return (
    <div className={cn('space-y-2', className)}>
      {items.map((item, index) => {
        ...
        const translateX = swipeState ? swipeState.currentX - swipeState.startX : 0
        ...
        return (
          <div
            ...
            style={{
              transform: swipeState?.isSwiping ? `translateX(${translateX}px)` : undefined
            }}
      ...
      <div className="flex items-center justify-around space-x-2">
        {actions.map((action, index) => (
          <Button
            ...
            className="flex-1 flex-col space-y-1 h-auto py-3 touch-manipulation"
          >
            {action.icon}
