...
export const OrderItemRow = React.memo<OrderItemRowProps>(
  ({ index, products, onRemove, onProductChange, orderType, transferMode }) => {
    ...
    const _isManualProduct = useWatch({
      control: form.control,
    ...
    const watchedManualProductName = useWatch({
      control: form.control,
    ...
    const [productOverride, setProductOverride] =
    ...
    }, [resolvedProduct?.inventory?.batches, currentBatchNumber]);
          ...
          <div className="flex h-8 items-center text-xs text-gray-700">
            {watchedProductId ? (
              <span className="truncate">
                {products.find(p => p.id === watchedProductId)?.name || '-'}
              </span>
            ) : watchedManualProductName ? (
              <span className="truncate">{watchedManualProductName}</span>
            ) : (
