...
// Icons
import { Save, ArrowLeft, ShoppingCart, AlertCircle, Loader2, User, FileText } from 'lucide-react'
...
// API and Types
import { createSalesOrder, updateSalesOrder, salesOrderQueryKeys } from '@/lib/api/sales-orders'
import { SalesOrder, SalesOrderCreateInput, SalesOrderUpdateInput } from '@/lib/types/sales-order'
import { 
...
export function SalesOrderForm({ mode, initialData, onSuccess, onCancel }: SalesOrderFormProps) {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      setSubmitError(error instanceof Error ? error.message : '创建销售订单失败')
    }
  })
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: salesOrderQueryKeys.lists() })
      queryClient.invalidateQueries({ queryKey: salesOrderQueryKeys.detail(response.data.id) })
      if (onSuccess) {
        onSuccess(response.data)
      } else {
        router.push('/sales-orders')
    ...
    onError: (error) => {
      setSubmitError(error instanceof Error ? error.message : '更新销售订单失败')
    }
  })
  ...
  // 表单提交
  const onSubmit = async (data: SalesOrderCreateFormData | SalesOrderUpdateFormData) => {
    setSubmitError('')
  ...
  const getAvailableStatuses = () => {
    ...
    const availableStatuses = SALES_ORDER_STATUS_TRANSITIONS[currentStatus] || []
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {isEdit && initialData && (
                  <div className="md:col-span-2">
                    ...
                    <p className="font-mono text-lg font-medium">{initialData.orderNumber}</p>
                  </div>
                )}
              </div>
