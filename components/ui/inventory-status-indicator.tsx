...
// 状态图标映射
const STATUS_ICONS: Record<InventoryStatus, React.ComponentType<{ className?: string }>> = {
...
const InventoryStatusIndicator = React.forwardRef<HTMLDivElement, InventoryStatusIndicatorProps>(
  ({
    className,
    status,
    size,
...
const InventoryHealth = React.forwardRef<HTMLDivElement, InventoryHealthProps>(
  ({ items, className, ...props }, ref) => {
    ...
    const healthStats = React.useMemo(() => {
      ...
      const danger = items.filter(item => ['out_of_stock', 'damaged', 'expired'].includes(item.status)).length
    ...
    return (
      <div className={cn("space-y-4", className)} ref={ref} {...props}>
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-sm">库存健康度</h3>
          <div className="flex items-center gap-2">
            <div className={cn(
              "text-2xl font-bold",
              healthStats.healthScore >= 80 ? "text-green-600" :
              healthStats.healthScore >= 60 ? "text-yellow-600" :
              "text-red-600"
            )}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-green-50 border border-green-200 rounded-lg p-3">
            ...
            <div className="text-lg font-bold text-green-700">{healthStats.safe}</div>
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            ...
            <div className="text-lg font-bold text-yellow-700">{healthStats.warning}</div>
          </div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-3">
            ...
            <div className="text-lg font-bold text-red-700">{healthStats.danger}</div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
            ...
            <div className="text-lg font-bold text-slate-700">{healthStats.total}</div>
          </div>
        </div>
...
const QuickStatusToggle = React.forwardRef<HTMLDivElement, QuickStatusToggleProps>(
  ({
    currentStatus,
    onStatusChange,
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive ? inventoryStatusVariants({ status }) : "border border-border bg-background"
              )}
            >
              <StatusIcon className="w-3 h-3" />
