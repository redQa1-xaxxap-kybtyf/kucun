...
export interface TileSpecification {
  ...
  // 表面处理
  surface?: 'glossy' | 'matte' | 'textured' | 'polished' | 'natural' | 'antique'
...
const SpecificationDisplay = React.forwardRef<HTMLDivElement, SpecificationDisplayProps>(
  ({ 
    className, 
    variant, 
    size, 
    ...
    if (compact) {
      return (
        <div className={cn("flex flex-wrap gap-1", className)} ref={ref} {...props}>
          {length && width && (
    ...
    return (
      <div className={cn("space-y-3", className)} ref={ref} {...props}>
        ...
        <div className="flex flex-wrap gap-2">
          {length && width && (
            <div className={cn(specificationDisplayVariants({ variant: "size", size }))}>
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>
              </svg>
              <span>{length}×{width}mm</span>
            </div>
          )}
          
          {thickness && (
            <div className={cn(specificationDisplayVariants({ variant: "thickness", size }))}>
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-wrap gap-2">
          {surface && (
            <div className={cn(specificationDisplayVariants({ variant: "surface", size }))}>
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {grade && (
            <div className={cn(specificationDisplayVariants({ variant: "grade", size }))}>
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showAll && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
            ...
            {frostResistance !== undefined && (
              <div className="flex items-center gap-1 text-muted-foreground">
                <span>抗冻:</span>
                <span className="font-medium">{frostResistance ? '是' : '否'}</span>
              </div>
            )}
...
const SpecificationCard = React.forwardRef<HTMLDivElement, SpecificationCardProps>(
...
const SpecificationCompare = React.forwardRef<HTMLDivElement, SpecificationCompareProps>(
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b">
              <th className="text-left p-2 font-medium">规格项目</th>
              {specifications.map((spec) => (
                <th key={spec.id} className="text-left p-2 font-medium min-w-32">
                  {spec.name}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            ...
            <tr className="border-b">
              ...
              {specifications.map((spec) => (
                <td key={spec.id} className="p-2">
                  {spec.specification.thickness ? `${spec.specification.thickness}mm` : '-'}
                </td>
              ))}
            </tr>
            <tr className="border-b">
              ...
              {specifications.map((spec) => (
                <td key={spec.id} className="p-2">
                  {spec.specification.surface ? SURFACE_LABELS[spec.specification.surface] : '-'}
                </td>
              ))}
            </tr>
            <tr className="border-b">
              ...
              {specifications.map((spec) => (
                <td key={spec.id} className="p-2">
                  {spec.specification.grade ? GRADE_LABELS[spec.specification.grade] : '-'}
                </td>
              ))}
            </tr>
            <tr className="border-b">
