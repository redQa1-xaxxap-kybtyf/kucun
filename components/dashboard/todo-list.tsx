...
const TodoItemComponent = React.forwardRef<HTMLDivElement, TodoItemProps>(
  ({ todo, onComplete, onView, compact = false, showCheckbox = true, className, ...props }, ref) => {
      ...
      return (
        ...
        >
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            <IconComponent className={cn("h-4 w-4", `text-${typeConfig.color}-500`)} />
          ...
          <div className="min-w-0 flex-1 space-y-2">
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-2 min-w-0 flex-1">
                <IconComponent className={cn("h-4 w-4 flex-shrink-0", `text-${typeConfig.color}-500`)} />
                <h4 className={cn(
                  "font-medium text-sm",
                  todo.status === 'completed' && "line-through"
                )}>
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              ...
              <span>
                {dashboardUtils.formatTimeAgo(todo.createdAt)}
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  ...
                  {todoStats.overdue > 0 && `${todoStats.urgent > 0 ? ', ' : ''}${todoStats.overdue} 逾期`}
