...
> {
  ...
  return items.map(item => {
    ...
    const trimmedDisplayName =
      item.displayName?.trim() || (product?.name ?? trimmedManualName ?? '未知商品');
    const trimmedSpecification = item.specification?.trim() ?? product?.specification ?? undefined;
    const trimmedUnit = item.unit?.trim() ?? product?.unit ?? 'piece';
    const itemWeight = typeof item.weight === 'number' ? item.weight : product?.weight ?? undefined;
    const trimmedProductCode = item.productCode?.trim() || product?.code || '';

    return {
      productId:
        item.productId && item.productId.trim().length > 0 ? item.productId : null,
      ...
      manualProductName: item.isManualProduct ? trimmedManualName ?? '临时商品' : undefined,
      ...
      manualUnit: item.isManualProduct ? item.manualUnit?.trim() ?? undefined : undefined,
