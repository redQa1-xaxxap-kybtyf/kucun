...
export function SalesOrderPrintTemplate({ order }: Props) {
  ...
  return (
    <div className="sales-order-print-template" style={{
      width: '297mm',
      minHeight: '210mm',
      padding: '10mm',
      ...
      <div style={{
        ...
        <div style={{ flex: '0 0 auto' }}>
          客户：<span style={{ fontWeight: 'bold' }}>{order.customer?.name || '未知客户'}</span>
        </div>
      ...
      <table style={{
        ...
        <tbody>
          {(order.items ?? []).map((item, _index) => {
            // 从备注中提取每件片数信息
            const piecesPerPackage = item.remarks?.match(/(\d+)片\/件/)?.[1] ||
            ...
            return (
              <tr key={item.id} style={{ height: '35px' }}>
                ...
                <td style={tableCellStyle}>{item.specification || item.product?.specification || '-'}</td>
                <td style={tableCellStyle}>{item.displayUnit || item.product?.unit || '-'}</td>
                <td style={tableCellStyle}>{item.displayQuantity || item.quantity}</td>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            合计金额（大写）：<span style={{ textDecoration: 'none' }}>
              {numberToChinese(totalAmount)}元整
            </span>
          </div>
          <div style={{ display: 'flex', gap: '50px', alignItems: 'center' }}>
            <span>合计数量：{totalQuantity}</span>
            <span>总重量：<span style={{ color: '#00aa00', fontWeight: 'bold' }}>
              {calculateTotalWeight(order.items)}吨
            </span></span>
            <span>合计金额：<span style={{ color: '#ff0000', fontWeight: 'bold' }}>
