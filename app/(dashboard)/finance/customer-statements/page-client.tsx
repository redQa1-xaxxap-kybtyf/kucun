...
export function CustomerStatementsPageClient({
  ...
  const fallbackTotals = statements.reduce(
    (totals, statement: CustomerStatementListItem) => ({
      receivable:
        totals.receivable +
        statement.summary.receivables.receivableBalance,
      payable:
        totals.payable + statement.summary.payables.payableBalance,
  ...
  const totalNetBalance =
    statisticsData?.totalNetBalance ?? fallbackTotals.net;
  ...
  const formatBalance = (balance: number) => {
    ...
    return <span className="text-[hsl(var(--color-text-tertiary))]">{formatCurrency(0)}</span>;
  };
                ...
                <TableBody>
                  {statements.map((statement: CustomerStatementListItem) => (
                    <TableRow key={statement.customerId}>
                      ...
                      <TableCell>{statement.customerPhone || '-'}</TableCell>
