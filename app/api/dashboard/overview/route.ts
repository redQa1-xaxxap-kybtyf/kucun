...
export async function GET(request: NextRequest) {
  try {
    ...
    // 计算销售指标
    const currentRevenue = currentSalesOrders.reduce((sum, order) => sum + (order.totalAmount || 0), 0)
    const previousRevenue = previousSalesOrders.reduce((sum, order) => sum + (order.totalAmount || 0), 0)
    ...
    const businessOverview = {
      ...
      inventory: {
        ...
        stockHealth: Math.max(0, 100 - (lowStockProducts + outOfStockProducts * 2) * 10),
      },
      purchase: {
