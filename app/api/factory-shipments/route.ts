...
export async function GET(request: NextRequest) {
  try {
    ...
    // 验证查询参数
    const validatedParams = factoryShipmentOrderListParamsSchema.parse(queryParams);
    const { page, pageSize, status, customerId, containerNumber, orderNumber, startDate, endDate } = validatedParams;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { containerNumber, customerId, status, totalAmount, receivableAmount, depositAmount, remarks, planDate, items } = validatedData;
    ...
    // 计算订单总金额
    const calculatedTotalAmount = items.reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0);
