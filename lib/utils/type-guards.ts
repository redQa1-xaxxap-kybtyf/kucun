...
export function isNavigationItem(item: unknown): item is NavigationItem {
  ...
  return (
    ...
    (nav.badge === undefined || typeof nav.badge === 'string' || typeof nav.badge === 'number') &&
...
export function isNotificationItem(item: unknown): item is NotificationItem {
  ...
  return (
    ...
    ['info', 'warning', 'error', 'success'].includes(notification.type as string) &&
...
export function isBreadcrumbItem(item: unknown): item is BreadcrumbItem {
  ...
  return (
    ...
    (breadcrumb.isCurrent === undefined || typeof breadcrumb.isCurrent === 'boolean')
  )
}
 ...
 * 验证通知项数组
 */
export function validateNotificationItems(items: unknown[]): NotificationItem[] {
...
export class TypeSafeConverter {
   ...
   * 安全转换为导航项
   */
  static toNavigationItem(item: unknown, fallback?: Partial<NavigationItem>): NavigationItem | null {
    ...
    if (fallback && typeof item === 'object' && item !== null) {
      ...
      const converted = {
        ...
        badge: typeof obj.badge === 'string' || typeof obj.badge === 'number' ? obj.badge : fallback.badge,
        disabled: typeof obj.disabled === 'boolean' ? obj.disabled : fallback.disabled,
        requiredRoles: Array.isArray(obj.requiredRoles) ? obj.requiredRoles : fallback.requiredRoles
      }
...
export class TypeAssert {
   ...
   * 断言为导航项
   */
  static navigationItem(item: unknown, message?: string): asserts item is NavigationItem {
    if (!isNavigationItem(item)) {
   ...
   * 断言为通知项
   */
  static notificationItem(item: unknown, message?: string): asserts item is NotificationItem {
    if (!isNotificationItem(item)) {
   ...
   * 断言为侧边栏状态
   */
  static sidebarState(state: unknown, message?: string): asserts state is SidebarState {
   ...
   * 断言为布局配置
   */
  static layoutConfig(config: unknown, message?: string): asserts config is LayoutConfig {
    if (!isLayoutConfig(config)) {
