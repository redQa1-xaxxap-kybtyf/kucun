...
async function testCoreApis() {
  ...
  try {
    ...
    if (createCustomerResponse.ok) {
      ...
      console.log('   ⚠️  客户创建失败，状态码:', createCustomerResponse.status)
    }
    ...
    // 获取产品列表
    const getProductsResponse = await fetch(`${baseUrl}/api/products?page=1&limit=10`)
    ...
    if (productId) {
      ...
      if (createInboundResponse.ok) {
        ...
        console.log('   ✅ 入库记录创建成功，数量:', inboundResult.data.quantity)
      } else {
    ...
    // 获取销售订单列表
    const getOrdersResponse = await fetch(`${baseUrl}/api/sales-orders?page=1&limit=10`)
    if (getOrdersResponse.ok) {
      const ordersResult = await getOrdersResponse.json()
      console.log('   ✅ 销售订单列表获取成功，数量:', ordersResult.data.length)
    } else {
