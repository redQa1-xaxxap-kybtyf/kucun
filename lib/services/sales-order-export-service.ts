...
import { ExportService, type ExcelExportOptions, type ImageExportOptions } from './export-service';

/**
 * 销售订单图片导出配置
 */
export interface SalesOrderImageExportOptions extends Omit<ImageExportOptions, 'filename'> {
  /** 订单ID */
  orderId: string;
  /** 订单号 */
  orderNumber: string;
 ...
 * 销售订单Excel导出配置
 */
export interface SalesOrderExcelExportOptions extends Omit<ExcelExportOptions, 'filename'> {
  /** 订单ID */
  orderId: string;
  /** 订单号 */
  orderNumber: string;
...
export class SalesOrderExportService {
   ...
   * @returns Excel数据数组
   */
  private static prepareOrderExcelData(order: SalesOrderDetail): SalesOrderExcelData[] {
    ...
    return orderItems.map(item => {
      ...
      const grossProfitRate = unitPrice > 0 ? (grossProfit / unitPrice * 100) : 0;
   ...
   * @returns 摘要数据对象
   */
  private static prepareOrderSummaryData(order: SalesOrderDetail): Record<string, string | number> {
    ...
    return {
      ...
      '确认时间': '', // 订单表中没有确认时间字段
      '发货时间': order.shippedAt ? new Date(order.shippedAt).toLocaleString('zh-CN') : '',
      '产品数量': totalQuantity,
