...
// 入库类型枚举
export type InboundType = 'normal_inbound' | 'return_inbound' | 'adjust_inbound'

// 出库类型枚举
export type OutboundType = 'normal_outbound' | 'sales_outbound' | 'adjust_outbound'
...
export const INBOUND_TYPE_VARIANTS: Record<InboundType, 'default' | 'secondary' | 'destructive' | 'outline'> = {
  normal_inbound: 'default',
  return_inbound: 'secondary',
  adjust_inbound: 'outline'
}

export const OUTBOUND_TYPE_VARIANTS: Record<OutboundType, 'default' | 'secondary' | 'destructive' | 'outline'> = {
...
// 库存状态检查函数
export const getInventoryStatus = (inventory: Inventory, minQuantity: number = 10): {
...
// 格式化库存数量显示
export const formatInventoryQuantity = (inventory: Inventory, unit?: string): string => {
