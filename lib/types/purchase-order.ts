...
// 计算采购订单明细小计
export function calculatePurchaseOrderItemSubtotal(
  quantity: number,
  unitPrice: number
): number {
...
// 采购订单明细验证
export function validatePurchaseOrderItem(
  item: PurchaseOrderItemCreateInput | PurchaseOrderItemUpdateInput
): string[] {
  const errors: string[] = [];
...
export function validatePurchaseOrder(
  order: PurchaseOrderCreateInput | PurchaseOrderUpdateInput
): string[] {
  const errors: string[] = [];
