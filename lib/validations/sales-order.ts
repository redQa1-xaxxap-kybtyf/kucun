...
export const salesOrderCreateSchema = z.object({
  ...
  items: z.array(salesOrderItemCreateSchema)
    .min(1, '至少需要添加一个订单明细')
...
export const salesOrderUpdateSchema = z.object({
  ...
  remarks: baseValidations.remarks,
  items: z.array(salesOrderItemUpdateSchema)
    .min(1, '至少需要保留一个订单明细')
    .max(100, '订单明细不能超过100条')
    .optional(),
}).refine((data) => {
...
export const salesOrderSearchSchema = z.object({
  ...
  sortBy: z.enum(['orderNumber', 'createdAt', 'updatedAt', 'totalAmount', 'status']).default('createdAt'),
...
export type SalesOrderItemCreateFormData = z.infer<typeof salesOrderItemCreateSchema>
export type SalesOrderItemUpdateFormData = z.infer<typeof salesOrderItemUpdateSchema>
export type SalesOrderStatusUpdateFormData = z.infer<typeof salesOrderStatusUpdateSchema>
...
// 计算订单总金额
export const calculateOrderTotal = (items: SalesOrderItemCreateFormData[] | SalesOrderItemUpdateFormData[]): number => {
  return Math.round(
    items
      .filter(item => !('_action' in item) || item._action !== 'delete')
      .reduce((total, item) => total + calculateItemSubtotal(item.quantity, item.unitPrice), 0) * 100
  ) / 100
}
...
// 订单状态流转验证
export const validateStatusTransition = (currentStatus: SalesOrderStatus, targetStatus: SalesOrderStatus): boolean => {
...
export const batchOperationSchema = z.object({
  ids: z.array(z.string().uuid('订单ID格式不正确'))
    .min(1, '请选择至少一个订单')
