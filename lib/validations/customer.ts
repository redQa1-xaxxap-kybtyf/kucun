import { z } from 'zod'
import { CustomerCreateInput, CustomerUpdateInput, CustomerQueryParams, CustomerExtendedInfo } from '@/lib/types/customer'
...
const extendedInfoValidations = {
  ...
  customerType: z.enum(['company', 'store', 'individual'], {
  ...
  tags: z.array(z.string().max(20, '标签长度不能超过20个字符'))
    .max(10, '标签数量不能超过10个')
    .optional(),
}
...
export const customerSearchSchema = z.object({
  ...
  parentCustomerId: z.string().uuid('上级客户ID格式不正确').optional().or(z.literal('')),
  customerType: z.enum(['company', 'store', 'individual']).optional().or(z.literal('')),
  level: z.enum(['A', 'B', 'C', 'D']).optional().or(z.literal('')),
  region: z.string().max(50, '区域不能超过50个字符').optional().or(z.literal('')),
  sortBy: z.enum(['name', 'createdAt', 'updatedAt', 'totalOrders', 'totalAmount']).default('createdAt'),
...
// 验证辅助函数
export const validateCustomerHierarchy = (customerId: string, parentCustomerId?: string): boolean => {
  // 防止自己作为自己的父级
...
// 扩展信息处理函数
export const processExtendedInfo = (extendedInfo?: CustomerExtendedInfo): string | undefined => {
...
// 解析扩展信息
export const parseExtendedInfo = (extendedInfoStr?: string): CustomerExtendedInfo => {
...
// 客户层级路径生成
export const generateCustomerPath = (customer: { id: string; parentCustomerId?: string }, allCustomers: { id: string; parentCustomerId?: string }[]): string[] => {
  const path: string[] = []
...
// 客户层级深度计算
export const calculateCustomerLevel = (customerId: string, allCustomers: { id: string; parentCustomerId?: string }[]): number => {
