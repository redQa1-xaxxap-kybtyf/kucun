...
export const payableRecordQuerySchema = z.object({
  ...
  startDate: z
    .string()
    .optional()
    .refine(date => !date || !isNaN(Date.parse(date)), '请输入有效的开始日期'),
  endDate: z
    .string()
    .optional()
    .refine(date => !date || !isNaN(Date.parse(date)), '请输入有效的结束日期'),
  sortBy: z
...
export const paymentOutRecordQuerySchema = z.object({
  ...
  endDate: z
    ...
    .optional()
    .refine(date => !date || !isNaN(Date.parse(date)), '请输入有效的结束日期'),
  sortBy: z.enum(['createdAt', 'paymentAmount', 'paymentDate']).optional(),
  sortOrder: z.enum(['asc', 'desc']).optional(),
});
...
export type CreatePaymentOutRecordData = z.infer<
  typeof createPaymentOutRecordSchema
>;
