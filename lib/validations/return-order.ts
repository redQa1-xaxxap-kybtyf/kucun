...
export const returnOrderItemSchema = z.object({
  ...
  // 退货数量不能超过原始数量
  return data.returnQuantity <= data.originalQuantity
}, {
...
export const createReturnOrderSchema = z.object({
  ...
  type: z.enum(['quality_issue', 'wrong_product', 'customer_change', 'damage_in_transit', 'other'] as const, {
...
// 退货订单更新验证规则
export const updateReturnOrderSchema = createReturnOrderSchema.partial().extend({
  id: z.string().min(1, '退货订单ID不能为空'),
})

// 退货状态更新验证规则
export const updateReturnStatusSchema = z.object({
  status: z.enum(['draft', 'submitted', 'approved', 'rejected', 'processing', 'completed', 'cancelled'] as const, {
...
// 退货搜索表单验证规则
export const returnOrderSearchSchema = z.object({
  search: z.string().max(100, '搜索关键词不能超过100字符').optional(),
  status: z.enum(['', 'draft', 'submitted', 'approved', 'rejected', 'processing', 'completed', 'cancelled'] as const).optional(),
  type: z.enum(['', 'quality_issue', 'wrong_product', 'customer_change', 'damage_in_transit', 'other'] as const).optional(),
  processType: z.enum(['', 'refund', 'exchange', 'repair', 'credit'] as const).optional(),
  customerId: z.string().optional(),
  salesOrderId: z.string().optional(),
  userId: z.string().optional(),
  startDate: z.string().optional(),
  endDate: z.string().optional(),
  sortBy: z.enum(['createdAt', 'returnNumber', 'totalAmount', 'status', 'submittedAt', 'completedAt']).default('createdAt'),
...
// 退货审核表单验证规则
export const returnOrderApprovalSchema = z.object({
  approved: z.boolean(),
  remarks: z.string()
  ...
  // 如果审核通过，必须设置退款金额
  if (data.approved && (data.refundAmount === undefined || data.refundAmount === null)) {
    return false
  }
  return true
}, {
...
export const batchReturnOrderSchema = z.object({
  ids: z.array(z.string().min(1, '退货订单ID不能为空'))
    .min(1, '请选择至少一个退货订单')
...
export type UpdateReturnStatusFormData = z.infer<typeof updateReturnStatusSchema>
export type ReturnOrderSearchFormData = z.infer<typeof returnOrderSearchSchema>
export type ReturnOrderApprovalFormData = z.infer<typeof returnOrderApprovalSchema>
 ...
 * 验证退货数量是否合理
 */
export function validateReturnQuantity(returnQuantity: number, originalQuantity: number, alreadyReturnedQuantity: number = 0): boolean {
  return returnQuantity > 0 && returnQuantity <= (originalQuantity - alreadyReturnedQuantity)
}
 ...
 * 验证退货状态流转
 */
export function validateStatusTransition(fromStatus: ReturnOrderStatus, toStatus: ReturnOrderStatus): boolean {
 ...
 * 计算退货明细小计
 */
export function calculateReturnItemSubtotal(quantity: number, unitPrice: number): number {
 ...
 * 计算退货订单总金额
 */
export function calculateReturnOrderTotal(items: ReturnOrderItemFormData[]): number {
 ...
 * 验证退货原因
 */
export function validateReturnReason(reason: string, type: ReturnOrderType): boolean {
