...
export const createPaymentRecordSchema = z.object({
  ...
  }).positive('收款金额必须大于0').max(999999999, '收款金额不能超过999,999,999'),
  
  paymentDate: z.string({
    required_error: '请选择收款日期',
    invalid_type_error: '收款日期必须是字符串'
  }).min(1, '请选择收款日期').refine((date) => {
...
export const updatePaymentRecordSchema = z.object({
  ...
  paymentAmount: z.number({
    invalid_type_error: '收款金额必须是数字'
  }).positive('收款金额必须大于0').max(999999999, '收款金额不能超过999,999,999').optional(),
...
// 收款记录查询验证规则
export const paymentRecordQuerySchema = z.object({
  ...
  sortBy: z.enum(['paymentDate', 'paymentAmount', 'createdAt']).optional().default('paymentDate'),
...
export const accountsReceivableQuerySchema = z.object({
  ...
  sortBy: z.enum(['orderDate', 'totalAmount', 'remainingAmount', 'overdueDays']).optional().default('orderDate'),
  sortOrder: z.enum(['asc', 'desc']).optional().default('desc')
}).refine((data) => {
  // 验证日期范围
  if (data.startDate && data.endDate) {
...
export type AccountsReceivableQueryInput = z.infer<typeof accountsReceivableQuerySchema>
export type PaymentConfirmationInput = z.infer<typeof paymentConfirmationSchema>
export type BatchPaymentOperationInput = z.infer<typeof batchPaymentOperationSchema>
export type PaymentStatisticsQueryInput = z.infer<typeof paymentStatisticsQuerySchema>

// 验证工具函数
export const validatePaymentAmount = (amount: number, maxAmount: number): boolean => {
...
export const validateBankInfo = (paymentMethod: string, bankInfo?: string): boolean => {
