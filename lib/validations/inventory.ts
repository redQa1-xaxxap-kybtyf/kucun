...
export const inboundCreateSchema = z.object({
  ...
  // 正常入库和退货入库需要单位成本
  if ((data.type === 'normal_inbound' || data.type === 'return_inbound') && !data.unitCost) {
    return false
  }
  return true
}, {
...
export const inventorySearchSchema = z.object({
  ...
  colorCode: z.string().max(20, '色号不能超过20个字符').optional().or(z.literal('')),
  lowStock: z.boolean().optional(),
  hasStock: z.boolean().optional(),
  productionDateStart: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '开始日期格式不正确').optional().or(z.literal('')),
  productionDateEnd: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '结束日期格式不正确').optional().or(z.literal('')),
  sortBy: z.enum(['productName', 'quantity', 'reservedQuantity', 'updatedAt']).default('updatedAt'),
  sortOrder: z.enum(['asc', 'desc']).default('desc'),
}).refine((data) => {
  // 验证日期范围
  if (data.productionDateStart && data.productionDateEnd) {
    return new Date(data.productionDateStart) <= new Date(data.productionDateEnd)
  }
  return true
}, {
...
// 入库记录搜索表单验证
export const inboundRecordSearchSchema = z.object({
  search: z.string().max(100, '搜索关键词不能超过100个字符').optional(),
  type: z.enum(['normal_inbound', 'return_inbound', 'adjust_inbound']).optional().or(z.literal('')),
  productId: z.string().uuid('产品ID格式不正确').optional().or(z.literal('')),
  userId: z.string().uuid('用户ID格式不正确').optional().or(z.literal('')),
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '开始日期格式不正确').optional().or(z.literal('')),
  endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '结束日期格式不正确').optional().or(z.literal('')),
  sortBy: z.enum(['createdAt', 'recordNumber', 'quantity', 'totalCost']).default('createdAt'),
  sortOrder: z.enum(['asc', 'desc']).default('desc'),
}).refine((data) => {
  // 验证日期范围
...
// 出库记录搜索表单验证
export const outboundRecordSearchSchema = z.object({
  search: z.string().max(100, '搜索关键词不能超过100个字符').optional(),
  type: z.enum(['normal_outbound', 'sales_outbound', 'adjust_outbound']).optional().or(z.literal('')),
  productId: z.string().uuid('产品ID格式不正确').optional().or(z.literal('')),
  customerId: z.string().uuid('客户ID格式不正确').optional().or(z.literal('')),
  salesOrderId: z.string().uuid('销售订单ID格式不正确').optional().or(z.literal('')),
  userId: z.string().uuid('用户ID格式不正确').optional().or(z.literal('')),
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '开始日期格式不正确').optional().or(z.literal('')),
  endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '结束日期格式不正确').optional().or(z.literal('')),
  sortBy: z.enum(['createdAt', 'recordNumber', 'quantity', 'totalCost']).default('createdAt'),
...
export type InboundRecordSearchFormData = z.infer<typeof inboundRecordSearchSchema>
export type OutboundRecordSearchFormData = z.infer<typeof outboundRecordSearchSchema>
...
export const batchOperationSchema = z.object({
  ids: z.array(z.string().uuid('记录ID格式不正确'))
    .min(1, '请选择至少一条记录')
