...
export const batchNumberSchema = z
  .string({
    ...
    invalid_type_error: '批次号必须是字符串',
  })
  .min(1, '批次号不能为空')
  .max(50, '批次号不能超过50个字符')
  .regex(/^[A-Z0-9\-_]+$/i, '批次号只能包含字母、数字、连字符和下划线');
...
export const batchSpecificationIdSchema = z
  .string({
    required_error: '批次规格参数ID不能为空',
    invalid_type_error: '批次规格参数ID必须是字符串',
  })
  .uuid('批次规格参数ID格式不正确');
