...
// 采购订单搜索验证规则
export const purchaseOrderSearchSchema = z
  .object({
...
// 采购订单状态更新验证规则
export const purchaseOrderStatusUpdateSchema = z
  .object({
    status: z.enum([
      'draft',
      'confirmed',
  ...
  .refine(
    data => {
      ...
      if (
        (data.status === 'received' || data.status === 'completed') &&
...
// 表单数据类型推断
export type PurchaseOrderCreateFormData = z.infer<
  typeof purchaseOrderCreateSchema
>;
export type PurchaseOrderUpdateFormData = z.infer<
  typeof purchaseOrderUpdateSchema
>;
export type PurchaseOrderSearchFormData = z.infer<
  typeof purchaseOrderSearchSchema
>;
export type PurchaseOrderStatusUpdateFormData = z.infer<
  typeof purchaseOrderStatusUpdateSchema
>;
export type PurchaseOrderItemFormData = z.infer<typeof purchaseOrderItemSchema>;
export type PurchaseOrderItemCreateFormData = z.infer<
...
export type PurchaseOrderBatchOperationFormData = z.infer<
  typeof purchaseOrderBatchOperationSchema
>;
