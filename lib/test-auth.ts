...
async function testAuthentication() {
  ...
  try {
    ...
    if (existingUser) {
      ...
      console.log(`   ✅ 创建测试用户成功: ${testUser.name} (${testUser.email})`)
    }
    ...
    // 测试正确密码
    const isValidPassword = await bcrypt.compare(testUserData.password, user.passwordHash)
    if (isValidPassword) {
      console.log('   ✅ 密码验证成功')
    } else {
    ...
    // 测试错误密码
    const isInvalidPassword = await bcrypt.compare('wrongpassword', user.passwordHash)
