...
export const purchaseOrderQueryKeys = {
  ...
  list: (params: PurchaseOrderQueryParams) => [...purchaseOrderQueryKeys.lists(), params] as const,
  ...
  supplier: (id: string) => [...purchaseOrderQueryKeys.suppliers(), id] as const,
}

// 获取采购订单列表
export async function getPurchaseOrders(params: PurchaseOrderQueryParams = {}): Promise<PurchaseOrderListResponse> {
...
// 创建采购订单
export async function createPurchaseOrder(data: PurchaseOrderCreateInput): Promise<PurchaseOrderResponse> {
...
// 更新采购订单
export async function updatePurchaseOrder(id: string, data: PurchaseOrderUpdateInput): Promise<PurchaseOrderResponse> {
  return apiRequest<PurchaseOrderResponse>(`${API_BASE_URL}/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  })
...
export async function deletePurchaseOrder(id: string): Promise<{ success: boolean; message?: string }> {
  return apiRequest<{ success: boolean; message?: string }>(`${API_BASE_URL}/${id}`, {
    method: 'DELETE',
  })
}
...
// 确认采购订单
export async function confirmPurchaseOrder(id: string): Promise<PurchaseOrderResponse> {
...
// 完成采购订单
export async function completePurchaseOrder(id: string, remarks?: string): Promise<PurchaseOrderResponse> {
  return updatePurchaseOrderStatus(id, 'completed', undefined, remarks)
}

// 取消采购订单
export async function cancelPurchaseOrder(id: string, remarks?: string): Promise<PurchaseOrderResponse> {
  return updatePurchaseOrderStatus(id, 'cancelled', undefined, remarks)
}

// 复制采购订单
export async function duplicatePurchaseOrder(id: string): Promise<PurchaseOrderResponse> {
...
// 导出采购订单
export async function exportPurchaseOrders(params: PurchaseOrderQueryParams = {}): Promise<Blob> {
  const searchParams = new URLSearchParams()
...
export async function batchOperatePurchaseOrders(
  ...
  return apiRequest<{ success: boolean; message?: string; results?: any[] }>(`${API_BASE_URL}/batch`, {
    method: 'POST',
...
// 供应商相关 API（简化版）
export async function getSuppliers(): Promise<{ success: boolean; data: Supplier[] }> {
...
export async function getSupplier(id: string): Promise<{ success: boolean; data: Supplier }> {
  return apiRequest<{ success: boolean; data: Supplier }>(`/api/suppliers/${id}`)
}
...
// 搜索供应商
export async function searchSuppliers(query: string): Promise<{ success: boolean; data: Supplier[] }> {
  const searchParams = new URLSearchParams({ search: query })
  return apiRequest<{ success: boolean; data: Supplier[] }>(`/api/suppliers/search?${searchParams.toString()}`)
}
...
export async function getSupplierPurchaseHistory(
  ...
  return apiRequest<PurchaseOrderListResponse>(`${API_BASE_URL}?${searchParams.toString()}`)
}
...
export async function createPurchaseOrderInbound(
  ...
  return apiRequest<{ success: boolean; message?: string }>(`${API_BASE_URL}/${purchaseOrderId}/inbound`, {
    method: 'POST',
