...
export const returnOrderQueryKeys = {
  ...
  list: (params: ReturnOrderQueryParams) => [...returnOrderQueryKeys.lists(), params] as const,
  ...
  salesOrderItems: (salesOrderId: string) => [...returnOrderQueryKeys.all, 'sales-order-items', salesOrderId] as const,
}
 ...
 * 获取退货订单列表
 */
export async function getReturnOrders(params: ReturnOrderQueryParams = {}): Promise<ReturnOrderListResponse> {
 ...
 * 创建退货订单
 */
export async function createReturnOrder(data: CreateReturnOrderFormData): Promise<ReturnOrderResponse> {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.message || `创建退货订单失败: ${response.statusText}`)
  }
 ...
 * 更新退货订单
 */
export async function updateReturnOrder(id: string, data: UpdateReturnOrderFormData): Promise<ReturnOrderResponse> {
  const response = await fetch(`${API_BASE}/${id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(errorData.message || `更新退货订单失败: ${response.statusText}`)
  }
...
export async function updateReturnOrderStatus(
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(errorData.message || `更新退货订单状态失败: ${response.statusText}`)
  }
 ...
 * 审核退货订单
 */
export async function approveReturnOrder(id: string, data: ReturnOrderApprovalFormData): Promise<ReturnOrderResponse> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(errorData.message || `审核退货订单失败: ${response.statusText}`)
  }
 ...
 * 删除退货订单
 */
export async function deleteReturnOrder(id: string): Promise<{ success: boolean; message?: string }> {
  const response = await fetch(`${API_BASE}/${id}`, {
    method: 'DELETE',
  })
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(errorData.message || `删除退货订单失败: ${response.statusText}`)
  }
 ...
 * 复制退货订单
 */
export async function duplicateReturnOrder(id: string): Promise<ReturnOrderResponse> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(errorData.message || `复制退货订单失败: ${response.statusText}`)
  }
 ...
 * 批量操作退货订单
 */
export async function batchReturnOrderOperation(data: BatchReturnOrderFormData): Promise<{ success: boolean; message?: string }> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(errorData.message || `批量操作失败: ${response.statusText}`)
  }
 ...
 * 获取销售订单可退货明细
 */
export async function getSalesOrderReturnableItems(salesOrderId: string): Promise<any> {
  const response = await fetch(`/api/sales-orders/${salesOrderId}/returnable-items`)
 ...
 * 导出退货订单
 */
export async function exportReturnOrders(params: ReturnOrderQueryParams = {}): Promise<Blob> {
  const searchParams = new URLSearchParams()
...
export function useReturnOrders(
  ...
  options?: Omit<UseQueryOptions<ReturnOrderListResponse>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
...
export function useReturnOrderStats(
  options?: Omit<UseQueryOptions<ReturnOrderStatsResponse>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
...
export function useCreateReturnOrder(
  options?: UseMutationOptions<ReturnOrderResponse, Error, CreateReturnOrderFormData>
) {
...
export function useUpdateReturnOrder(
  options?: UseMutationOptions<ReturnOrderResponse, Error, { id: string; data: UpdateReturnOrderFormData }>
) {
  const queryClient = useQueryClient()
  ...
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: returnOrderQueryKeys.detail(id) })
      queryClient.invalidateQueries({ queryKey: returnOrderQueryKeys.lists() })
      queryClient.invalidateQueries({ queryKey: returnOrderQueryKeys.stats() })
    },
    ...options,
...
export function useUpdateReturnOrderStatus(
  options?: UseMutationOptions<ReturnOrderResponse, Error, { id: string; status: string; remarks?: string; refundAmount?: number }>
) {
  const queryClient = useQueryClient()
  
  return useMutation({
    mutationFn: ({ id, status, remarks, refundAmount }) => updateReturnOrderStatus(id, status, remarks, refundAmount),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: returnOrderQueryKeys.detail(id) })
      queryClient.invalidateQueries({ queryKey: returnOrderQueryKeys.lists() })
      queryClient.invalidateQueries({ queryKey: returnOrderQueryKeys.stats() })
...
export function useApproveReturnOrder(
  options?: UseMutationOptions<ReturnOrderResponse, Error, { id: string; data: ReturnOrderApprovalFormData }>
) {
  const queryClient = useQueryClient()
...
export function useBatchReturnOrderOperation(
  options?: UseMutationOptions<{ success: boolean; message?: string }, Error, BatchReturnOrderFormData>
) {
  const queryClient = useQueryClient()
