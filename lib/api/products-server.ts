 ...
 */
export const getProductsForServer = cache(async (
  params: ProductListQueryParams
): Promise<PaginatedResponse<ServerProduct> | undefined> => {
  // 直接解析参数，移除不必要的 URLSearchParams 序列化
  const includeInventory = params.includeInventory ?? productConfig.defaultIncludeInventory;
  const includeStatistics = params.includeStatistics ?? productConfig.defaultIncludeStatistics;
  ...
  const status = params.status && (params.status as string) !== 'all' ? params.status : undefined;
  ...
  const cached = await getOrSetJSON(
    ...
    async () => {
      ...
      const formattedProducts = formatProductList({
        products: products as Parameters<typeof formatProductList>[0]['products'],
        inventoryMap,
        includeInventory,
        includeStatistics: finalIncludeStatistics,
      });
