...
export const dashboardQueryKeys = {
  ...
  salesTrend: (timeRange: TimeRange) => [...dashboardQueryKeys.all, 'sales-trend', timeRange] as const,
  inventoryTrend: (timeRange: TimeRange) => [...dashboardQueryKeys.all, 'inventory-trend', timeRange] as const,
  productRanking: (timeRange: TimeRange) => [...dashboardQueryKeys.all, 'product-ranking', timeRange] as const,
  customerRanking: (timeRange: TimeRange) => [...dashboardQueryKeys.all, 'customer-ranking', timeRange] as const,
  quickActions: () => [...dashboardQueryKeys.all, 'quick-actions'] as const,
  complete: (filters: DashboardFilters) => [...dashboardQueryKeys.all, 'complete', filters] as const,
}
...
export const dashboardApi = {
  ...
  // 获取业务概览
  getBusinessOverview: async (timeRange: TimeRange): Promise<BusinessOverview> => {
  ...
  getSalesTrend: async (timeRange: TimeRange): Promise<SalesTrendData> => {
    const response = await fetch(`${API_BASE}/sales-trend?timeRange=${timeRange}`)
    if (!response.ok) {
  ...
  // 获取库存趋势
  getInventoryTrend: async (timeRange: TimeRange): Promise<InventoryTrendData> => {
    const response = await fetch(`${API_BASE}/inventory-trend?timeRange=${timeRange}`)
    if (!response.ok) {
  ...
  // 获取产品销售排行
  getProductRanking: async (timeRange: TimeRange): Promise<ProductSalesRanking[]> => {
    const response = await fetch(`${API_BASE}/product-ranking?timeRange=${timeRange}`)
    if (!response.ok) {
  ...
  // 获取客户销售排行
  getCustomerRanking: async (timeRange: TimeRange): Promise<CustomerSalesRanking[]> => {
    const response = await fetch(`${API_BASE}/customer-ranking?timeRange=${timeRange}`)
    if (!response.ok) {
