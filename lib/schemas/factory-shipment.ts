...
export const factoryShipmentOrderItemSchema = z.object({
  ...
  manualProductName: z.string().max(100, '商品名称不能超过100个字符').optional().or(z.literal('')),
  manualSpecification: z.string().max(200, '规格不能超过200个字符').optional().or(z.literal('')),
  manualWeight: z.number().min(0, '重量不能为负数').optional(),
  manualUnit: z.string().max(20, '单位不能超过20个字符').optional().or(z.literal('')),
  
  // 通用显示字段
  displayName: z.string().min(1, '商品名称不能为空').max(100, '商品名称不能超过100个字符'),
  specification: z.string().max(200, '规格不能超过200个字符').optional().or(z.literal('')),
  unit: z.string().min(1, '单位不能为空').max(20, '单位不能超过20个字符'),
  weight: z.number().min(0, '重量不能为负数').optional(),
  
  remarks: z.string().max(500, '备注不能超过500个字符').optional().or(z.literal('')),
}).refine(
  (data) => {
    // 如果是手动输入商品，必须填写商品名称
...
export const createFactoryShipmentOrderSchema = z.object({
  containerNumber: z.string().min(1, '集装箱号码不能为空').max(50, '集装箱号码不能超过50个字符'),
  ...
  remarks: z.string().max(1000, '备注不能超过1000个字符').optional().or(z.literal('')),
  planDate: z.date().optional(),
  items: z.array(factoryShipmentOrderItemSchema).min(1, '至少需要添加一个商品'),
}).refine(
  (data) => {
    // 定金不能超过应收金额
    if (data.depositAmount && data.receivableAmount && data.depositAmount > data.receivableAmount) {
      return false;
    }
    return true;
  },
...
export const updateFactoryShipmentOrderSchema = z.object({
  containerNumber: z.string().min(1, '集装箱号码不能为空').max(50, '集装箱号码不能超过50个字符').optional(),
  ...
  items: z.array(factoryShipmentOrderItemSchema).min(1, '至少需要添加一个商品').optional(),
}).refine(
  (data) => {
    // 定金不能超过应收金额
    if (data.depositAmount && data.receivableAmount && data.depositAmount > data.receivableAmount) {
    ...
    // 已付金额不能超过应收金额
    if (data.paidAmount && data.receivableAmount && data.paidAmount > data.receivableAmount) {
      return false;
    }
    return true;
  },
...
// 厂家发货订单列表查询参数验证
export const factoryShipmentOrderListParamsSchema = z.object({
  page: z.number().int().min(1, '页码必须大于0').optional(),
  pageSize: z.number().int().min(1, '每页数量必须大于0').max(100, '每页数量不能超过100').optional(),
  status: factoryShipmentStatusSchema.optional(),
  customerId: z.string().uuid('客户ID格式不正确').optional(),
  containerNumber: z.string().max(50, '集装箱号码不能超过50个字符').optional(),
...
// 类型推断
export type CreateFactoryShipmentOrderData = z.infer<typeof createFactoryShipmentOrderSchema>;
export type UpdateFactoryShipmentOrderData = z.infer<typeof updateFactoryShipmentOrderSchema>;
export type FactoryShipmentOrderListParams = z.infer<typeof factoryShipmentOrderListParamsSchema>;
export type UpdateFactoryShipmentOrderStatusData = z.infer<typeof updateFactoryShipmentOrderStatusSchema>;
export type FactoryShipmentOrderItemData = z.infer<typeof factoryShipmentOrderItemSchema>;
