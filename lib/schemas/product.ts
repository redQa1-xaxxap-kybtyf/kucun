 ...
 * 产品单位枚举
 */
export const ProductUnitEnum = z.enum(['piece', 'sheet', 'strip', 'box', 'square_meter'])
...
export function formatProductSpecification(specification?: string): string {
  ...
  return specification.length > 50 ? `${specification.substring(0, 50)}...` : specification
}

/**
 * 计算产品总重量
 */
export function calculateTotalWeight(weight?: number, quantity?: number): number {
