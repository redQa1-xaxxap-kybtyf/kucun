...
class AuthService {
    ...
    async login(credentials) {
        try {
            // 登录请求不需要Token
            const response = await (0, request_1.post)(api_1.API_ENDPOINTS.AUTH.LOGIN, credentials, { needAuth: false });
            // 保存Token和用户信息
            if (response.token) {
    ...
    async register(data) {
        try {
            const response = await (0, request_1.post)(api_1.API_ENDPOINTS.AUTH.REGISTER, data, { needAuth: false });
            // 注册成功后自动保存Token
            if (response.token) {
                wx.setStorageSync(api_1.TOKEN_KEY, response.token);
                wx.setStorageSync('user_info', response.user);
