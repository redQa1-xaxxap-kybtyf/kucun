...
class UserService {
   ...
   */
  async getFavorites() {
    const list = await (0, request_1.get)(
      api_1.API_ENDPOINTS.PROFILE.FAVORITES
    );
    return (list || []).map(item => ({
      ...item,
      thumbnailUrl: item.thumbnailUrl
        ? (0, media_1.appendMiniTokenForLocalUploads)(item.thumbnailUrl)
        : item.thumbnailUrl,
    }));
  }
  /**
   ...
   */
  async getHistory() {
    const list = await (0, request_1.get)(api_1.API_ENDPOINTS.PROFILE.HISTORY);
    return (list || []).map(item => ({
      ...item,
      thumbnailUrl: item.thumbnailUrl
        ? (0, media_1.appendMiniTokenForLocalUploads)(item.thumbnailUrl)
  ...
  async addHistory(productId) {
    ...
    await (0, request_1.post)(
      api_1.API_ENDPOINTS.PROFILE.HISTORY,
      { productId },
      {
