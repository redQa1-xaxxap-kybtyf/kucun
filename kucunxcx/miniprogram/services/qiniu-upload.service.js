...
class QiniuUploadService {
    ...
    async getUploadParams(fileName, type = 'product', kind) {
        try {
            ...
            const response = await (0, request_1.post)(api_1.API_ENDPOINTS.UPLOAD.QINIU_TOKEN, {
                type,
                kind,
                fileName,
            });
    ...
    async uploadToQiniu(filePath, params, onProgress) {
        ...
        for (const host of uploadHosts) {
            try {
                const result = await this.doUpload(host, filePath, uploadToken, key, onProgress);
                // 上传成功，返回结果
                return {
                    key,
                    url, // 使用后端返回的完整 URL
    ...
    async uploadImage(filePath, kind, options = {}) {
        const { type = 'product', onProgress, showLoading: shouldShowLoading = true, } = options;
        try {
            // 显示加载提示
    ...
    async uploadFiles(filePaths, kind, options = {}) {
        ...
        for (let i = 0; i < filePaths.length; i += CONCURRENCY) {
            ...
            const batchResults = await Promise.all(batch.map((filePath, index) => this.uploadImage(filePath, kind, {
                ...options,
