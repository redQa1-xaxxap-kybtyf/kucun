    ...
    const items = (response.data || []).map(product => {
      ...
      if (Array.isArray(product.images)) {
        images = product.images
          ...
          .filter(url => !!url)
          .map(url => (0, media_1.appendMiniTokenForLocalUploads)(url));
      }
      // 计算统一的“每件片数”
      // 1) 优先使用产品本身的 piecesPerUnit（>1 时认为是有效包装）
      // 2) 如未设置或为 1，则从批次规格 / 库存批次中推导（所有批次一致时采用）
      let effectivePiecesPerUnit =
      ...
      return {
        ...
        thumbnailUrl: (0, media_1.appendMiniTokenForLocalUploads)(
    ...
    if (Array.isArray(raw.images)) {
      raw.images.forEach(img => {
        ...
        if (img && typeof img.url === 'string' && img.url) {
          const url = (0, media_1.appendMiniTokenForLocalUploads)(img.url);
          if (img.type === 'effect') {
    ...
    if (
      ...
      try {
        const inventoryResponse =
          await inventory_service_1.default.getInventoryList({
            productId: id,
            page: 1,
            limit: 100,
          });
