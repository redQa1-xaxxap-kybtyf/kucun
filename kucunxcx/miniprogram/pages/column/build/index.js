...
const column_service_1 = __importDefault(require("../../../services/column.service"));
const column_calculator_1 = require("../../../utils/column-calculator");
const app = getApp();
/** 默认面型配置 */
const DEFAULT_FACE_TYPES = [
    (0, column_calculator_1.createDefaultFaceType)('front', '正面', 400, 1),
];
Page({
    data: {
        /** 目标高度（mm） */
        ...
        delta: 0,
        /** 差值状态 */
        deltaStatus: { status: 'empty', message: '请选择素材以开始拼柱' },
        /** 面型配置列表 */
        faceTypes: [],
        /** 当前编辑的面型索引 */
        ...
        /** 位置列表 */
        slots: [
            { type: 'CAP', name: '盖帽', icon: '🔝' },
            { type: 'HEAD', name: '柱头', icon: '🏛️' },
            { type: 'BODY', name: '柱身', icon: '📦' },
            { type: 'BASE', name: '底座', icon: '🔻' },
        ],
        /** 是否可以生成清单 */
        canGenerate: false,
    },
    onLoad() {
        ...
        if (faceTypes[0].materials.BODY.segments === 0) {
            const recommendedSegments = (0, column_calculator_1.calculateRecommendedBodySegments)(state.targetHeight);
    ...
    recalculateHeight() {
        ...
        const buildHeight = (0, column_calculator_1.calculateTotalHeight)(faceTypes);
        const delta = (0, column_calculator_1.calculateDelta)(targetHeight, buildHeight);
        const deltaStatus = (0, column_calculator_1.getDeltaStatus)(delta, buildHeight);
        const canGenerate = (0, column_calculator_1.areAllFaceTypesComplete)(faceTypes);
        this.setData({
            buildHeight,
            delta,
            deltaStatus,
    ...
    async openMaterialPicker(slot) {
        ...
        try {
            ...
            const materials = await column_service_1.default.getMaterialsBySlot(slot, currentFaceType.width);
            this.setData({
    ...
    onSegmentChange(e) {
        ...
        const faceType = { ...updatedFaceTypes[currentFaceTypeIndex] };
        faceType.materials = { ...faceType.materials };
        const current = faceType.materials[slot];
        ...
        faceType.materials[slot] = { ...current, segments: newSegments };
        updatedFaceTypes[currentFaceTypeIndex] = faceType;
    ...
    async onGenerateScheme() {
        ...
        try {
            const response = await column_service_1.default.generateScheme({
                targetHeight,
                faceTypes,
            });
            // 保存结果到全局状态
     ...
     */
    getSlotName(slot) {
        return (0, column_calculator_1.getSlotName)(slot);
    },
});
