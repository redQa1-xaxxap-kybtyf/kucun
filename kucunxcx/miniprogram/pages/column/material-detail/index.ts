...
import type { Material, SlotType } from '../../../types/column';
...
Page({
   ...
   * 加载素材详情
   */
  async loadMaterialDetail(id: string) {
    this.setData({ loading: true });

    try {
      const material = await columnService.getMaterialDetail(id);
      this.setData({
        material,
  ...
  async toggleFavorite() {
    ...
    try {
      if (isFavorite) {
        await columnService.removeFavorite(materialId);
        wx.showToast({ title: '已取消收藏', icon: 'success' });
      } else {
        await columnService.addFavorite(materialId);
  ...
  onAddToScheme() {
    ...
    if (fromBuild && targetSlot) {
      ...
      wx.showActionSheet({
        ...
        success: async res => {
          const slots: SlotType[] = ['BODY', 'BASE', 'CAP'];
          ...
          if (material.slot !== selectedSlot) {
            wx.showModal({
              title: '提示',
              content: `该素材是${material.slotName}，是否确认添加为${getSlotName(selectedSlot)}？`,
              success: modalRes => {
                if (modalRes.confirm) {
   ...
   * 查看相关产品
   */
  onViewRelated(e: WechatMiniprogram.TouchEvent) {
