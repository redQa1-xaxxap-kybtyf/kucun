...
Page({
  ...
  // 加载指定产品的所有效果图
  async loadProductEffects(productId: string, targetImageUrl?: string) {
    try {
      // 获取产品详情
  ...
  // 检查是否已收藏
  checkFavorite(imageUrl: string): boolean {
    const favorites: string[] = wx.getStorageSync(FAVORITES_KEY) || [];
