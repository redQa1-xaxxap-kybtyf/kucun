...
Page({
    ...
    async loadCategories() {
        ...
        try {
            ...
            const totalProducts = categories.reduce((sum, cat) => sum + (cat.productCount || (cat._count?.products ?? 0)), 0);
            this.setData({
                categories: treeData,
    ...
    onSearch() {
        ...
        const filteredCategories = this.data.allCategories
            ...
            // 过滤匹配的子分类
            const matchedChildren = cat.children?.filter(child => child.name.toLowerCase().includes(keyword) ||
