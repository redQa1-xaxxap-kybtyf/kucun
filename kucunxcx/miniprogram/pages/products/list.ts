...
Page({
  data: {
    ...
    // 筛选选项（文案尽量短，避免被省略号截断）
    categoryOptions: [{ text: '分类', value: '0' }] as DropdownOption[],
    ...
    // 布局模式
    layoutMode: 'list' as 'list' | 'grid-2' | 'grid-3',
  ...
  async onLoad(options: any) {
    ...
    if (
      ...
      this.setData({
        layoutMode: savedLayoutMode as 'list' | 'grid-2' | 'grid-3',
      });
    } else {
      // 默认使用双列模式（最佳实践）
  ...
  switchToLayout(e: any) {
    const targetMode = e.currentTarget.dataset.mode as
      | 'list'
      | 'grid-2'
      | 'grid-3';
