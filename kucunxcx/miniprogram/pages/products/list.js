...
Page({
  data: {
    ...
    // 筛选选项
    categoryOptions: [
      { text: '全部', value: '0', icon: '' },
      { text: '瓷砖', value: '1', icon: '' },
      { text: '地板', value: '2', icon: '' },
      { text: '墙砖', value: '3', icon: '' },
      { text: '装饰材料', value: '4', icon: '' },
    ],
    statusOptions: [
      { text: '全部', value: '0', icon: '' },
      { text: '有货', value: '1', icon: '' },
      { text: '缺货', value: '2', icon: '' },
      { text: '预警', value: '3', icon: '' },
    ],
    sortOptions: [
      { text: '默认', value: '0', icon: '' },
      { text: '库存↓', value: '1', icon: '' },
      { text: '库存↑', value: '2', icon: '' },
      { text: '名称', value: '3', icon: '' },
    ],
    // 产品列表
    products: [],
    // 分页
    ...
    try {
      ...
      const enrichedOptions = categories
        .map(cat => {
          ...
          return { text, value: cat.id, icon: '' };
        })
        .sort((a, b) => a.text.localeCompare(b.text));
      const categoryOptions = [
        { text: '全部分类', value: '0', icon: '' },
        ...enrichedOptions,
      ];
      this.setData(
        {
  ...
  // 根据当前筛选分类更新分享标题/路径
  updateShareInfo() {
    const { filterCategory, allCategories } = this.data;
    // 默认：全部列表
    let shareTitle = '产品列表';
