...
const qiniu_upload_service_1 = __importDefault(
  require('../../services/qiniu-upload.service')
);
...
Page({
  ...
  async loadProductDetail(id) {
    try {
      const product =
        await product_service_1.productService.getProductDetail(id);
      // 回填表单数据
      this.setData({
        code: product.code,
        name: product.name,
        ...
        if (fileInfo && typeof fileInfo.size === 'number') {
          ...
          if (sizeMb > maxSizeMb) {
            ...
            wx.compressImage({
              ...
              success: compRes => {
                const compressedPath = compRes?.tempFilePath || filePath;
  ...
  async uploadImage(filePath, kind) {
    try {
      // 使用七牛云服务上传
      const result = await qiniu_upload_service_1.default.uploadImage(
        filePath,
        kind
      );
      // 上传成功，更新对应的状态
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (this.data.isEditMode && this.data.productId) {
        product = await product_service_1.productService.updateProduct(
          this.data.productId,
          payload
        );
        wx.showToast({
        ...
        if (this.data.isEditMode) {
          try {
            ...
            channel?.emit('productUpdated', {
              id: product?.id || this.data.productId,
            });
          } catch (_error) {
            // ignore
          }
