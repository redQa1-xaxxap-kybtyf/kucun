...
Page({
    ...
    onLoad(options) {
        ...
        if (options.id) {
            ...
            const canEdit = auth_service_1.default.canEditProduct();
            this.setData({
    ...
    async loadProductDetail() {
        ...
        try {
            // 调用产品详情API
            const product = await product_service_1.default.getProductDetail(this.data.productId);
            // 统一格式化“最后更新”时间，避免直接展示 ISO 字符串
            const normalized = {
                ...product,
                updatedAt: product.updatedAt
                    ? (0, format_1.formatDateTime)(product.updatedAt)
                    : product.updatedAt,
            };
            ...
            try {
                await user_service_1.default.addHistory(product.id);
            }
            catch (historyError) {
    ...
    // 解析并缓存当前产品的 1 级分类名称
    async resolveRootCategory(product) {
        try {
    ...
    async onFavorite() {
        ...
        try {
            const result = await user_service_1.default.toggleFavorite(product.id);
            wx.showToast({
