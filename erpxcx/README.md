...
- 首页按花色/系列找产品，并展示规格、图片数量和效果案例提示。
- 同一花色内切换罗马柱、转角石、线条、外墙砖、配套。
- 产品详情支持查看型号、规格、包装、重量、效果案例、同色配套和同色花色货架。
- 产品详情支持生成产品海报并保存到相册。
- 产品组详情页支持从分享页返回同色花色货架，可选规格使用纵向列表展示。
- 产品详情页支持从分享页返回同色花色货架，并支持缩略图快速切换主图。
- 产品组/产品/首页分享与转发。
- 管理员登录，接入 ERP `/api/auth/mini-login`。
- 管理员工作台。
- 管理端库存查询，支持关键词和扫码查库存。
- 管理端产品维护，支持搜索、新增、编辑、图片上传、上架、下架和带原因的删除保护。
