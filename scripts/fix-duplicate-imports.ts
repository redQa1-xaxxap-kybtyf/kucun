        ...
        if (match) {
          ...
          if (importMap.has(module)) {
            ...
            if (
              ...
              !existingImports.some(imp => imp.includes('type '))
            ) {
      ...
      for (const [module, importsList] of importMap) {
        if (importsList.length === 1) {
          ...
          if (regularImports.length > 0 && typeImports.length > 0) {
            // 有类型导入和普通导入，需要合并
            const regularParts = regularImports
              .join(', ')
              .replace(/[{}]/g, '')
              .split(',')
              .map(s => s.trim());
            const typeParts = typeImports
              .join(', ')
              .replace(/type\s+/g, '')
              .replace(/[{}]/g, '')
              .split(',')
            ...
            const allParts = [
              ...regularParts,
              ...typeParts.map(p => `type ${p}`),
            ];
            newLines.push(
              `import { ${allParts.join(', ')} } from '${module}';`
            );
          } else if (regularImports.length > 0) {
            const parts = regularImports
              .join(', ')
              .replace(/[{}]/g, '')
              .split(',')
              .map(s => s.trim());
            ...
            const parts = typeImports
              ...
              .map(s => s.trim());
            newLines.push(
              `import type { ${parts.join(', ')} } from '${module}';`
            );
          }
        }
      }
