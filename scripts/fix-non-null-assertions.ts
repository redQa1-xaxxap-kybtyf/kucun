...
function fixNonNullAssertion(fix: NonNullAssertionFix): boolean {
  try {
    ...
    if (targetLine.includes(fix.pattern)) {
      ...
      console.log(`✅ 修复非空断言: ${fix.file}:${fix.line} - ${fix.description}`);
      return true;
    } else {
      console.log(`⚠️  未找到目标模式: ${fix.file}:${fix.line} - ${fix.pattern}`);
      return false;
    }
  } catch (error) {
...
function fixGenericNonNullAssertions(filePath: string): boolean {
  try {
    ...
    // 替换方法调用
    newContent = newContent.replace(/(\w+)!\.(\w+)\(/g, (match, obj, method) => {
      modified = true;
      return `${obj}?.${method}(`;
    });
