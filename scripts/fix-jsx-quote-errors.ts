...
function fixJsxQuoteErrors(filePath: string): boolean {
  try {
    ...
    // 修复被错误修改的className属性
    newContent = newContent.replace(/className="([^"]*?)&quot;([^"]*?)"/g, (match, before, after) => {
      modified = true;
      return `className="${before}"${after}"`;
    });
    
    // 修复被错误修改的其他属性
    newContent = newContent.replace(/(\w+)="([^"]*?)&quot;([^"]*?)"/g, (match, attr, before, after) => {
      // 跳过已经正确的HTML实体
    ...
    newContent = newContent.replace(/`([^`]*?)&quot;([^`]*?)`/g, (match, before, after) => {
      // 只修复明显错误的情况
      if (!before.includes('title') && !before.includes('aria-') && !before.includes('alt')) {
        modified = true;
        return `\`${before}"${after}\``;
      }
      return match;
    ...
    newContent = newContent.replace(/>\s*([^<]*?)&quot;([^<]*?)\s*</g, (match, before, after) => {
