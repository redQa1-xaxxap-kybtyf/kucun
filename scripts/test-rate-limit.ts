...
async function testBasicRateLimit() {
  ...
  const config = getRateLimitConfig(RateLimitType.LOGIN);
  const limiter = createRateLimiter(storage, config);
...
async function testMultipleIdentifiers() {
  ...
  const storage = new MemoryRateLimitStorage();
  const config = getRateLimitConfig(RateLimitType.AUTH);
  const limiter = createRateLimiter(storage, config);
...
async function testReset() {
  ...
  const storage = new MemoryRateLimitStorage();
  const config = getRateLimitConfig(RateLimitType.WRITE);
  const limiter = createRateLimiter(storage, config);
...
async function testGetStatus() {
  ...
  const storage = new MemoryRateLimitStorage();
  const config = getRateLimitConfig(RateLimitType.READ);
  const limiter = createRateLimiter(storage, config);
...
async function testMemoryStorageStats() {
  ...
  const storage = new MemoryRateLimitStorage();
  const config = getRateLimitConfig(RateLimitType.GLOBAL);
  const limiter = createRateLimiter(storage, config);
