...
async function testImprovedCodeGeneration() {
  ...
  const testCategories = [
    { name: '瓷砖', description: '测试瓷砖分类的编码生成', expectedCode: 'CERAMIC_TILES' },
    { name: '地砖', description: '测试地砖分类的编码生成', expectedCode: 'FLOOR_TILES' },
    { name: '墙砖', description: '测试墙砖分类的编码生成', expectedCode: 'WALL_TILES' },
    { name: '抛光砖', description: '测试抛光砖分类的编码生成', expectedCode: 'POLISHED_TILES' },
    { name: '石材', description: '测试石材分类的编码生成', expectedCode: 'STONE_MATERIALS' },
    { name: '辅材', description: '测试辅材分类的编码生成', expectedCode: 'AUXILIARY_MATERIALS' },
    { name: '工具', description: '测试工具分类的编码生成', expectedCode: 'TOOLS' },
    { name: `Test Category ${timestamp}`, description: '测试英文分类名称', expectedCode: 'TEST_CATEGORY' },
    { name: `混合Test分类${timestamp}`, description: '测试中英文混合名称', expectedCode: 'CERAMIC_TILES' }, // 应该匹配到瓷砖关键词
    { name: `特殊@#$符号${timestamp}`, description: '测试特殊字符过滤', expectedCode: /^[A-Z_]+$/ },
  ];
  ...
  try {
    for (let i = 0; i < testCategories.length; i++) {
      ...
      console.log(`   期望编码模式: ${typeof testCase.expectedCode === 'string' ? testCase.expectedCode : testCase.expectedCode.toString()}`);
      ...
      if (createResult.success) {
        ...
        console.log(`   🔍 编码格式: ${isValidFormat ? '✅ 符合标准（纯英文大写+数字+下划线）' : '❌ 不符合标准'}`);
        
        // 验证编码匹配
        if (typeof testCase.expectedCode === 'string') {
          const isExpectedMatch = generatedCode === testCase.expectedCode || generatedCode.startsWith(testCase.expectedCode);
          console.log(`   🎯 编码匹配: ${isExpectedMatch ? '✅ 符合预期' : '❌ 不符合预期'}`);
        } else {
          const isPatternMatch = testCase.expectedCode.test(generatedCode);
          console.log(`   🎯 编码模式: ${isPatternMatch ? '✅ 符合预期' : '❌ 不符合预期'}`);
        }
    ...
    if (duplicateResult1.success) {
      console.log(`   ✅ 第一个瓷砖分类创建成功，编码: "${duplicateResult1.data.code}"`);
      ...
      if (duplicateResult2.success) {
        console.log(`   ✅ 第二个瓷砖分类创建成功，编码: "${duplicateResult2.data.code}"`);
        console.log(`   🔍 编码唯一性: ${duplicateResult1.data.code !== duplicateResult2.data.code ? '✅ 通过' : '❌ 失败'}`);
