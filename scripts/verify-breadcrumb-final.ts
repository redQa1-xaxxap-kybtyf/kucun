...
function verifyBreadcrumbComponent(): VerificationResult[] {
  ...
  try {
    const breadcrumbPath = join(process.cwd(), 'components/common/Breadcrumb.tsx');
    const content = readFileSync(breadcrumbPath, 'utf-8');
    
    // 验证1: 检查是否包含产品详情逻辑
    const hasProductDetail = content.includes('产品详情') && 
    ...
    // 验证2: 检查是否包含分类详情逻辑
    const hasCategoryDetail = content.includes('分类详情') && 
    ...
    // 验证3: 检查是否包含客户详情逻辑
    const hasCustomerDetail = content.includes('客户详情') && 
    ...
    // 验证4: 检查编辑页面逻辑
    const hasEditLogic = content.includes('编辑产品') && 
    ...
    const hasOldLogic = content.includes('详情 #${segment.slice(0, 8)}') &&
    ...
    // 验证6: 检查PATH_TITLES扩展
    const hasExtendedPaths = content.includes("'/products/edit': '编辑产品'") &&
...
function verifyDashboardIntegration(): VerificationResult[] {
  ...
  try {
    ...
    // 检查是否启用了面包屑
    const hasBreadcrumbEnabled = content.includes('showBreadcrumb={true}') ||
...
export { verifyBreadcrumbComponent, verifyProjectStructure, verifyDashboardIntegration };
