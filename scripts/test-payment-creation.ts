...
async function testPaymentCreation() {
  ...
  // 检查是否提供了真实的ID
  if (salesOrderId.includes('PLEASE_REPLACE') || customerId.includes('PLEASE_REPLACE')) {
    console.error('❌ 错误: 请提供真实的订单ID和客户ID');
    console.log('\n使用方法:');
    console.log('npx tsx scripts/test-payment-creation.ts [订单ID] [客户ID] [金额]');
    console.log('\n示例:');
    console.log('npx tsx scripts/test-payment-creation.ts "cm62a5vup0003qbnkqh1gwsf6" "ck62a5vup0001qbnk8h2fwsf7" 1000');
    ...
    if (!response.ok) {
      ...
      if (result.details) {
        ...
        result.details.forEach((detail: any, index: number) => {
          console.error(`  ${index + 1}. 字段: ${detail.path?.join('.') || '未知'}`);
      ...
      // 如果创建成功，尝试确认收款
      console.log('\n⏳ 尝试确认收款...');
      const confirmResponse = await fetch(`${baseUrl}/api/payments/${result.data.id}/confirm`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
console.log('  npx tsx scripts/test-payment-creation.ts [订单ID] [客户ID] [金额]');
console.log('\n示例:');
console.log('  npx tsx scripts/test-payment-creation.ts "cm62abc" "ck62def" 1000');
console.log('\n前置条件:');
