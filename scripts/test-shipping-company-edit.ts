...
function logResult(step: string, status: 'success' | 'failed', message: string, data?: unknown) {
...
async function testShippingCompanyEdit() {
  ...
  try {
    // 步骤 1: 获取订单列表
    console.log('\n📋 步骤 1: 获取厂家发货订单列表');
    const listResponse = await fetch(`${baseUrl}/api/factory-shipments?page=1&limit=10`);
    
    if (!listResponse.ok) {
      logResult('获取订单列表', 'failed', `HTTP ${listResponse.status}: ${listResponse.statusText}`);
      return;
    }
    
    const listData = await listResponse.json();
    
    if (!listData.success || !listData.data?.orders || listData.data.orders.length === 0) {
      logResult('获取订单列表', 'failed', '没有找到订单数据');
      return;
    }
    
    logResult('获取订单列表', 'success', `找到 ${listData.data.orders.length} 个订单`);
    ...
    if (!updateResponse.ok) {
      ...
      logResult('更新船公司名称', 'failed', `HTTP ${updateResponse.status}: ${JSON.stringify(errorData)}`);
      return;
    }
    ...
    // 步骤 4: 验证更新
    console.log('\n🔍 步骤 4: 验证更新结果');
    const verifyResponse = await fetch(`${baseUrl}/api/factory-shipments?page=1&limit=10`);
    ...
    const updatedOrder = verifyData.data?.orders?.find((o: { id: string }) => o.id === orderId);
    ...
    if (originalShippingCompany !== '(空)') {
      ...
      if (restoreResponse.ok) {
        logResult('恢复原始数据', 'success', `已恢复为: ${originalShippingCompany}`);
      } else {
    ...
    logResult('测试执行', 'failed', `异常: ${error instanceof Error ? error.message : String(error)}`);
  }
