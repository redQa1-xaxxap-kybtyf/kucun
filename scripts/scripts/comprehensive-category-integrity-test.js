...
exports.testDataIntegrity = testDataIntegrity;
exports.testDataConsistency = testDataConsistency;
exports.testBoundaryConditions = testBoundaryConditions;
exports.testDataSync = testDataSync;
exports.testErrorRecovery = testErrorRecovery;
...
async function testDataIntegrity() {
  ...
  async function testForeignKeyConstraints() {
    try {
      // 检查孤立的产品记录
      const orphanProducts = await db_1.prisma.$queryRaw`
  ...
  async function testDataTypesAndFormats() {
    try {
      const issues = [];
      // 检查状态值
      const invalidStatuses = await db_1.prisma.category.findMany({
        where: {
          status: {
      ...
      // 检查空值
      const nullNames = await db_1.prisma.category.count({
        where: { name: null },
      });
      if (nullNames > 0) {
        issues.push({
      ...
      // 检查空编码
      const nullCodes = await db_1.prisma.category.count({
        where: { code: null },
      });
      if (nullCodes > 0) {
        issues.push({
...
async function testDataConsistency() {
  ...
  async function testTransactionHandling() {
    try {
      ...
      // 检查是否有部分数据被创建（不应该有）
      const checkCategory = await db_1.prisma.category.findFirst({
...
async function testDataSync() {
  ...
  async function testCacheDbConsistency() {
    try {
      ...
      // 直接从数据库获取数据
      const dbData = await db_1.prisma.category.findUnique({
        where: { id: categoryId },
        include: {
          parent: true,
          children: true,
...
async function testErrorRecovery() {
  ...
  async function testTransactionRollback() {
    try {
      ...
      // 检查是否有部分数据被创建
      const codeExists = await db_1.prisma.category.findFirst({
  ...
  async function testDataIntegrityUnderError() {
    try {
      // 获取测试前的数据计数
      const beforeCount = await db_1.prisma.category.count();
      // 尝试多个无效操作
      ...
      // 检查数据计数是否没有变化
      const afterCount = await db_1.prisma.category.count();
