...
async function testToastImportFixes() {
  ...
  try {
    for (const filePath of filesToCheck) {
      ...
      // 检查1：是否还有sonner导入
      const hasSonnerImport = content.includes(
      ...
      results.push({
        ...
        details: hasSonnerImport
          ? "发现: import { toast } from 'sonner'"
          : undefined,
      });

      // 检查2：是否有useToast导入
      const hasUseToastImport = content.includes(
      ...
      results.push({
        ...
        details: hasUseToastImport
          ? "发现: import { useToast } from '@/hooks/use-toast'"
          : undefined,
      });
      ...
      results.push({
        ...
        message: hasUseToastCall
          ? '正确调用useToast hook'
          : '缺少useToast hook调用',
        details: hasUseToastCall
          ? '发现: const { toast } = useToast()'
          : undefined,
      });

      // 检查4：toast使用格式是否正确
      const hasCorrectToastUsage =
      ...
      results.push({
        ...
        message: hasCorrectToastUsage
          ? '使用正确的toast格式'
          : 'toast格式可能不正确',
        details: hasCorrectToastUsage
          ? '发现正确的toast({ variant: ... })格式'
          : undefined,
      });

      // 检查5：是否有错误的toast调用（如toast.success, toast.error）
      const hasIncorrectToastCalls =
      ...
      results.push({
        ...
        message: hasIncorrectToastCalls
          ? '发现错误的toast调用'
          : '无错误的toast调用',
        details: hasIncorrectToastCalls
          ? '发现: toast.success/error/info等调用'
          : undefined,
      });
    }
    ...
    if (useToastExists) {
      ...
      const exportsUseToast = useToastContent.includes(
        'export { useToast, toast }'
      );
      results.push({
        name: 'useToast导出检查',
        success: exportsUseToast,
        message: exportsUseToast
          ? 'useToast正确导出'
          : 'useToast导出可能有问题',
      });
    }
    ...
    if (existsSync(layoutPath)) {
      ...
      const hasToasterImport = layoutContent.includes(
