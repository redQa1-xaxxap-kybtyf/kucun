...
async function checkSpecificFixes(): Promise<TestResult[]> {
  ...
  try {
    // 检查1：React 导入修复
    const productPageContent = require('fs').readFileSync('app/(dashboard)/products/[id]/page.tsx', 'utf8');
    const hasReactImport = productPageContent.includes("import React from 'react'");
    ...
    // 检查2：重复键值修复
    const categoryCodeContent = require('fs').readFileSync('lib/utils/category-code-generator.ts', 'utf8');
    ...
    // 检查3：入库页面常量定义
    const inboundPageContent = require('fs').readFileSync('app/(dashboard)/inventory/inbound/page.tsx', 'utf8');
    const hasInboundLabels = inboundPageContent.includes('INBOUND_REASON_LABELS');
    ...
    const productSchemaContent = require('fs').readFileSync('lib/schemas/product.ts', 'utf8');
    const hasCategoryId = productSchemaContent.includes('categoryId: z.string().optional()');
