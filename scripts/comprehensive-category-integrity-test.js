...
function logSkipped(category, name, reason) {
  testResults.total++;
  testResults.skipped++;
  testResults.details[category].total++;
...
async function retry(fn, maxRetries = TEST_CONFIG.maxRetries) {
...
async function testDataConsistency() {
  ...
  async function testConcurrentCreation() {
    try {
      ...
      logTest('consistency', '并发创建分类一致性测试', false, error.message);
    }
  }
  ...
  // 测试 2.3: 分类删除时的级联处理
  async function testDeleteCascadeHandling() {
    try {
      // 创建父分类
      const parentResponse = await apiRequest('/api/categories', {
        method: 'POST',
        body: JSON.stringify({
          name: `级联删除测试父级-${Date.now()}`,
          code: `DEL-P-${Date.now()}`,
        }),
      });
      ...
      // 检查删除前的状态
      const beforeDelete = await prisma.category.findUnique({
        where: { id: childId },
        select: { parentId: true },
      });

      // 尝试删除父分类（如果系统设置为SetNull，子分类的parentId应该变为null）
      const deleteResponse = await apiRequest(`/api/categories/${parentId}`, {
        method: 'DELETE',
      });
      ...
      const cascadeHandled =
        deleteResponse.response.ok &&
        (afterDelete?.parentId === null || afterDelete === null);
      ...
      logTest(
        ...
        '分类删除时的级联处理测试',
        cascadeHandled,
        !cascadeHandled ? '级联删除处理不正确' : null,
        {
          beforeDelete: beforeDelete?.parentId,
          afterDelete: afterDelete?.parentId,
          deleteSuccess: deleteResponse.response.ok,
        }
      );
    } catch (error) {
      logTest('consistency', '分类删除时的级联处理测试', false, error.message);
    }
  }
  ...
  async function testTransactionHandling() {
    try {
      ...
      logTest('consistency', '事务处理正确性测试', false, error.message);
    }
  }

  await testConcurrentCreation();
  await testParentChildConsistency();
  await testDeleteCascadeHandling();
...
async function testBoundaryConditions() {
  ...
  async function testThreeLevelLimit() {
    try {
      ...
      logTest('boundary', '3级分类限制严格执行测试', false, error.message);
    }
  }
  ...
  async function testStatusSwitching() {
    try {
      ...
      logTest('boundary', '分类状态切换一致性测试', false, error.message);
    }
  }

  // 测试 3.3: 排序顺序的唯一性
  async function testSortOrderUniqueness() {
    try {
      // 创建两个同级分类，使用相同的sortOrder
      const timestamp = Date.now();

      const cat1Response = await apiRequest('/api/categories', {
        method: 'POST',
        body: JSON.stringify({
          name: `排序测试1-${timestamp}`,
          code: `SORT1-${timestamp}`,
          sortOrder: 5,
        }),
      });

      const cat2Response = await apiRequest('/api/categories', {
        method: 'POST',
        body: JSON.stringify({
          name: `排序测试2-${timestamp}`,
          code: `SORT2-${timestamp}`,
          sortOrder: 5,
        }),
      });

      if (cat1Response.response.ok && cat2Response.response.ok) {
        testData.cleanupIds.push(cat1Response.data.data.id);
        testData.cleanupIds.push(cat2Response.data.data.id);

        // 检查是否都能成功创建（sortOrder可以重复，但系统应该能正确处理）
        logTest('boundary', '排序顺序唯一性测试', true, null, {
          bothCreated: true,
          cat1SortOrder: cat1Response.data.data.sortOrder,
          cat2SortOrder: cat2Response.data.data.sortOrder,
        });
      } else {
        logTest('boundary', '排序顺序唯一性测试', false, '无法创建测试分类');
      }
    } catch (error) {
      logTest('boundary', '排序顺序唯一性测试', false, error.message);
    }
  }

  await testThreeLevelLimit();
  await testStatusSwitching();
  await testSortOrderUniqueness();
}
...
async function testDataSync() {
  ...
  async function testCacheDbConsistency() {
    try {
      ...
      logTest('sync', '缓存与数据库一致性测试', false, error.message);
    }
  }

  // 测试 4.2: API响应数据的准确性
  async function testApiResponseAccuracy() {
    try {
      // 获取列表数据
      const listResponse = await apiRequest('/api/categories?limit=5');
      ...
      const categories = listResponse.data.data.slice(0, 3);
      let accuracyIssues = [];
      ...
      for (const category of categories) {
        ...
        if (detailResponse.data?.success && detailResponse.data.data) {
          const detailCategory = detailResponse.data.data;

          // 比较关键字段
          if (
            detailCategory.id !== category.id ||
            detailCategory.name !== category.name ||
            detailCategory.code !== category.code
          ) {
            accuracyIssues.push({
              categoryId: category.id,
              issue: 'List and detail API data mismatch',
              listData: {
                id: category.id,
                name: category.name,
                code: category.code,
      ...
      logTest(
        ...
        accuracyIssues.length > 0
          ? `发现 ${accuracyIssues.length} 个准确性问题`
          : null,
        { accuracyIssues }
      );
    } catch (error) {
      logTest('sync', 'API响应数据准确性测试', false, error.message);
    }
  }
  ...
  async function testConcurrentSync() {
    try {
      ...
      logTest('sync', '并发操作数据同步测试', false, error.message);
    }
  }

  await testCacheDbConsistency();
  await testApiResponseAccuracy();
...
async function testErrorRecovery() {
  ...
  async function testTransactionRollback() {
    try {
      ...
      logTest('recovery', '事务回滚机制测试', false, error.message);
    }
  }
  ...
  async function testDataIntegrityUnderError() {
    try {
      ...
      logTest('recovery', '错误状态下数据完整性测试', false, error.message);
    }
  }
  ...
  async function testPartialFailureRecovery() {
    try {
      // 创建一个正常的分类
      const normalResponse = await apiRequest('/api/categories', {
        method: 'POST',
        body: JSON.stringify({
          name: `恢复测试-${Date.now()}`,
          code: `RECOVER-${Date.now()}`,
        }),
      });

      if (!normalResponse.response.ok || !normalResponse.data?.data?.id) {
        throw new Error('无法创建正常分类');
      }

      const categoryId = normalResponse.data.data.id;
      testData.cleanupIds.push(categoryId);

      // 记录原始数据
      const originalData = normalResponse.data.data;

      // 尝试进行部分无效的更新
      const updateResponse = await apiRequest(`/api/categories/${categoryId}`, {
        method: 'PUT',
        body: JSON.stringify({
          name: 'x'.repeat(200), // 无效的长名称
          description: '有效的描述', // 有效的字段
        }),
      });

      // 检查原始数据是否没有被破坏
      const checkResponse = await apiRequest(`/api/categories/${categoryId}`);

      const dataIntact =
        checkResponse.data?.success &&
        checkResponse.data.data &&
        checkResponse.data.data.name === originalData.name;

      logTest(
        'recovery',
        '部分失败时数据恢复测试',
        !updateResponse.response.ok && dataIntact,
        updateResponse.response.ok || !dataIntact ? '部分失败恢复不当' : null,
        {
          updateFailed: !updateResponse.response.ok,
          dataIntact,
        }
      );
    } catch (error) {
      logTest('recovery', '部分失败时数据恢复测试', false, error.message);
    }
  }
