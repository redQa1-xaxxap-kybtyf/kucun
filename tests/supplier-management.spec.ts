...
test.describe('供应商管理功能', () => {
  ...
  test('应该显示供应商管理页面', async ({ page }) => {
    ...
    // 检查搜索框
    await expect(page.locator('input[placeholder*="搜索供应商"]')).toBeVisible();
  ...
  test('应该能够创建新供应商', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder="请输入供应商地址"]', '测试地址123号');
  ...
  test('应该验证表单输入', async ({ page }) => {
    ...
    // 验证长度验证错误
    await expect(page.locator('text=供应商名称不能超过100个字符')).toBeVisible();
  });
