...
describe('布局系统集成测试', () => {
  ...
  describe('认证布局系统', () => {
    ...
    it('应该检查用户权限', () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/auth/error?error=AccessDenied')
    })
  })
  ...
  describe('响应式设计测试', () => {
    it('应该在桌面端显示侧边栏', () => {
      ...
      useMediaQuery.mockImplementation((query: string) => {
        ...
        if (query === '(min-width: 769px) and (max-width: 1024px)') return false
        return true
      })
  ...
  describe('移动端手势操作测试', () => {
    ...
    it('应该支持滑动关闭菜单', async () => {
      ...
      render(
        <MobileNav open={true} onOpenChange={mockOnOpenChange} />
      )
