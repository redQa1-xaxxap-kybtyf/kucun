...
describe('ColorCodeDisplay', () => {
  ...
  it('does not handle click when not interactive', () => {
    const handleClick = jest.fn();
    render(
      <ColorCodeDisplay colorCode="W001" onColorCodeClick={handleClick} />
    );
  ...
  it('applies correct variant classes', () => {
    const { rerender } = render(
      <ColorCodeDisplay colorCode="W001" variant="outline" />
    );
    ...
    expect(screen.getByText('W001').parentElement).toHaveClass(
      'border-slate-200'
    );
  });
  ...
  it('applies correct size classes', () => {
    const { rerender } = render(
      <ColorCodeDisplay colorCode="W001" size="sm" />
    );
...
describe('ColorCodeGrid', () => {
  ...
  it('highlights selected color code', () => {
    render(
      <ColorCodeGrid colorCodes={testColorCodes} selectedColorCode="W001" />
    );
  ...
  it('applies correct grid columns', () => {
    render(<ColorCodeGrid colorCodes={testColorCodes} columns={2} />);
  ...
  it('renders empty grid when no color codes provided', () => {
    ...
    // 网格容器应该存在但为空
    const gridContainer =
      screen.getByText('W001').closest('div') || document.createElement('div');
